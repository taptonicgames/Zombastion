...
public class ObjectParabolaJumpHelper
{
	...
	{
		if (data.randomRotation) RotateRandom(data);
		...
		foreach (var item in data.list)
		{
			...
			Vector3[] waypoints = new Vector3[] { centerPoint, bezierPointA, bezierPointB, targetPos, bezierPointC, bezierPointD };
			index++;
			int remain = data.list.Count - index;

			item.DOPath(waypoints, 0.6f, PathType.CubicBezier, PathMode.Full3D, 10, Color.red).SetEase(Ease.Linear).OnComplete(() =>
			{
				if (data.hideOnFinish) item.gameObject.SetActive(false);
				if (remain == 0 && data.destroyOnFinish) DestroyItems(data.list);
	...
	{
		foreach (Transform item in list) Obj.Destroy(item.gameObject);
	}
	...
	public struct JumpObjectData
	{
		...
		public bool randomRotation, destroyOnFinish, hideOnFinish;
